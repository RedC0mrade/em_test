...
from app.core.sql_models import BookAlchemyModel, BorrowAlchemyModel
...
async def create_borrows(borrows_in: CreateBorrows,
    ...
    if book.available_copies == 0:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                            detail=f"The book with id {borrows_in.book_id} is not available.")
...
async def get_all_borrows(session: AsyncSession) -> List[BorrowAlchemyModel]:
    ...
    stmt = select(BorrowAlchemyModel).options(joinedload(BorrowAlchemyModel.book))
...
async def patch_borrows(id: int,
    ...
    if borrow.return_date:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail=f"The book has already been returned in {borrow.return_date}.")
